...
class ZenseServiceServicerRGBDIR(zense_pb2_grpc.ZenseServiceServicer):
    ...
    def SendRGBDImage(self, request, context):
        ...
        rgb_img_pb = zense_pb2.Image(height=self.rgb_image.shape[0],
                                     width=self.rgb_image.shape[1],
                                     timestamp=timestamp,
                                     channel=3,
                                     data=ndarray_to_bytes(self.rgb_image))

        depth_img_pb = zense_pb2.Image(height=self.depth_image.shape[0],
        ...
        return zense_pb2.ImageRGBDReply(image_rgb=rgb_img_pb,
                                        image_depth=depth_img_pb)

    def SendRGBDIRImage(self, request, context):
        global zense
        if not self.is_rgb:

            return zense_pb2.ImageRGBDIRReply()
        while not zense.update():
            pass
        ...
        ir_img_pb = zense_pb2.Image(height=self.ir_image.shape[0],
        ...
        return zense_pb2.ImageRGBDIRReply(image_rgb=rgb_img_pb,
                                          image_ir=ir_img_pb,
                                          image_depth=depth_img_pb)
    ...
    def SetLaserIntensity(self, request, context):
        global zense
        status = zense.set_pulse_count(request.intensity)
        return zense_pb2.SetLaserIntensityReply(is_success=status)

    def GetLaserIntensity(self, request, context):
        global zense
        intensity = zense.get_pulse_count()
        return zense_pb2.GetLaserIntensityReply(intensity=intensity)

    def SetDepthRange(self, request, context):
        global zense
        given_range = request.given_range
        status = zense.set_depth_range(given_range.encode('utf-8'))
        return zense_pb2.SetDepthRangeReply(is_success=status)
